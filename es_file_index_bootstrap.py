...
from typing import Dict, List, Optional, Tuple, Union #, LiteralString
...
# When either es_upserts or es_deletes reaches this size, flush them to ElasticSearch
# immediately rather than waiting for the end of the dataset loop. This limits memory
# usage and reduces the risk of losing work if the process is interrupted.
...
# Set to True when indexing into a known-empty ElasticSearch index (e.g. initial bootstrap).
# Skips the get_docs_from_es() call per dataset, eliminating unnecessary ES queries.
# Set to False for normal incremental runs where stale documents may need to be deleted.
BOOTSTRAP_EMPTY_INDEX = True
...
BOOTSTRAP_CHECKPOINT_FILE = "exec_info/bootstrap_checkpoint.txt"
...
def index_published_datasets(
    ...
    with driver.session() as neo4j_session:
        ...
        for raw_record in datasets:
            ...
            if BOOTSTRAP_EMPTY_INDEX:
                # Index is known to be empty — skip ES query and delete-diff entirely.
                es_filepath_map = {}
            else:
                try:
            ...
            for idx, (rel_path, local_file) in enumerate(local_filepath_map.items()):
                ...
                # flush to ES if either list has reached FLUSH_ES_DOCS_LEVEL
                if len(es_upserts) >= FLUSH_ES_DOCS_LEVEL or len(es_deletes) >= FLUSH_ES_DOCS_LEVEL:
                    try:
                        logger.info(
                            f"Flushing {len(es_upserts)} upserts and {len(es_deletes)} deletes to ES "
                        ...
                        flushed_ids = [f"{doc['dataset_uuid']}/{doc['rel_path']}" for doc in es_upserts]
                        ...
                        logger.error(
                            f"Error flushing documents to ES for dataset {dataset['uuid']}: {e}"
                        )
                        num_errors += 1
            ...
            if es_upserts or es_deletes:
                try:
                    logger.info(
                        f"Flushing {len(es_upserts)} remaining upserts and {len(es_deletes)} remaining deletes "
                    ...
                    logger.error(f"Error flushing remaining documents for dataset {dataset['uuid']}: {e}")
                    num_errors += 1
