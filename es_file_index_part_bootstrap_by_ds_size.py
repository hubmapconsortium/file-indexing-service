...
# keep them updated.  It extends es_file_index_part_bootstrap.py by adding a dataset size filter so that large       #
# datasets (those with many files) can be processed separately from small ones.                                      #
#                                                                                                                    #
# Like its parent, this program works with a UUID partition of the SQLite database built by local_file_index.py.     #
# In addition, DS_FILE_COUNT_THRESHOLD and INDEX_LARGE_DATASETS control whether this instance processes datasets     #
# above or below the threshold.  A full bootstrap requires 8 runs: 4 partition keys x 2 size thresholds.            #
#                                                                                                                    #
# Datasets whose dataset_uuid is NULL (not identified during the file system crawl) are always processed in the      #
# below-threshold runs, since their file count is unknown.                                                           #
#                                                                                                                    #
# After all runs complete, the multiple ES indices will be merged so the needed indices of                           #
# file-indexing-service are bootstrapped.                                                                            #
######################################################################################################################
import sys
import ast
import hashlib
...
def load_datasets_by_file_count(db: Database) -> dict:
    """Query SQLite for file counts per dataset_uuid.
    Returns a dict mapping dataset_uuid -> file_count.
    Datasets with NULL dataset_uuid are excluded since they cannot be matched by uuid."""
    query = """
        SELECT dataset_uuid, COUNT(*) as file_count
        FROM files
        WHERE dataset_uuid IS NOT NULL
...
def main():
    ...
    with (
        ...
        # load file counts per dataset from SQLite for size-based filtering
        global datasets_by_file_count
        datasets_by_file_count = load_datasets_by_file_count(db)
